    ...
    for code, (name, land_id) in sorted(LAENDER.items()):
        ...
        if anon:
            L += ["### Oberklasse ist ein anonymer OWL-Ausdruck", "",
                  ...
                  "enthalten dürfen und wo sie hängen, zeigt `scripts/klasse.sh <ID>`. Der Explorer "
                  "leitet seit Version 0.4.0 aus dem benannten Glied der anonymen "
                  "`owl:intersectionOf` eine `subClassOf`-Kante ab (A ⊑ (B ⊓ R) ⇒ A ⊑ B) — "
                  "SHACL-Shapes der benannten Gliedklasse (z. B. „Element (SH)“) greifen deshalb "
                  "trotzdem. Unsichtbar bleibt nur eine Klasse, deren Oberklassen-Ausdruck gar kein "
                  "benanntes Glied enthält; `scripts/klasse.sh <ID>` zeigt, welcher Fall vorliegt.",
                  "", "| ID | Klasse | Hinweis (editorialNote) |", "|---|---|---|"]
